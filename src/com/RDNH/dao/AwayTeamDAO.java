 ...
 *
 */
public class AwayTeamDAO {
	connectionPool cp;
	Connection conn;
	public AwayTeamDAO(connectionPool cp){
		this.cp=cp;
		try {
	 ...
	 * @throws SQLException
	 */
	public void insertAwayTeam(int game,String[] line) throws SQLException {
		...
		String sql = "insert into AwayTeam(game,number,place,name,isFirst,time,goal,assists,threatball,shoot,shootOnTarget,shootOnTargetRate,foul,fouled,save)VALUES('"+game+"','"+number+"','"+line[1]+"','"+line[2]+"','"+isFirst+"','"+time+"','"+goal+"','"+assists+"','"+threatball+"','"+shoot+"','"+shootOnTarget+"','"+shootOnTargetRate+"','"+foul+"','"+fouled+"','"+save+"')";           
		try {
	 ...
	 * @return
	 */
	public ArrayList<Player> getAwayTeam(int game){
		ResultSet ret = null;
		PreparedStatement stmt=null;
		ArrayList<Player> player=new ArrayList<Player>();
		String sql ="select number,name,place,isFirst,time,goal,shoot from AwayTeam where game='"+game+"'";
		try {
			stmt = conn.prepareStatement(sql, ResultSet.TYPE_SCROLL_SENSITIVE,ResultSet.CONCUR_READ_ONLY);
			stmt.execute(sql);
